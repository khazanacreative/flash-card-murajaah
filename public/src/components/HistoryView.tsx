...
import { AssessmentResult, Mufradat, getLevelInfo } from '@/utils/scoring';
...
export function HistoryView({ results, mufradatList, onSelectMufradat }: HistoryViewProps) {
  if (results.length === 0) {
    return (
      <div className="card-islamic flex flex-col items-center justify-center rounded-xl p-8 text-center">
        <div className="mb-4 text-6xl">📝</div>
        <h3 className="mb-2 text-lg font-semibold text-foreground">Belum ada riwayat</h3>
        <p className="text-sm text-muted-foreground">
          Mulai penilaian untuk melihat riwayat di sini
        </p>
      </div>
    );
  }

  const getMufradatById = (id: string) => mufradatList.find((m) => m.id === id);
  const getMufradatIndex = (id: string) => mufradatList.findIndex((m) => m.id === id);

  return (
    <div className="space-y-3">
      <h3 className="text-center text-sm font-semibold text-muted-foreground">
        Riwayat Penilaian ({results.length} soal)
      </h3>

      <div className="space-y-2">
        {results.map((result, index) => {
          const mufradat = getMufradatById(result.mufradatId);
          if (!mufradat) return null;

          const levelInfo = getLevelInfo(mufradat.level);
          const allCorrect = result.membaca && result.mengartikan && result.kalimat;
          const mufradatIndex = getMufradatIndex(result.mufradatId);

          return (
            <button
              key={result.mufradatId}
              onClick={() => onSelectMufradat?.(mufradatIndex)}
              className="card-islamic group w-full rounded-lg p-4 text-left transition-all hover:shadow-elevated"
            >
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-muted-foreground">{mufradat.meaning}</p>
                </div>
      ...
      {/* Summary */}
      <div className="card-islamic rounded-lg p-4">
